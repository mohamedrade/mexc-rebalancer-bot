...
def main_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("ℹ️ كيف يعمل البوت", callback_data="menu:info")],
    ])
...
def allocs_list_kb(allocations: List[Dict]) -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton("🔙 الإعدادات", callback_data="menu:settings")])
...
def portfolios_list_kb(portfolios: List[Dict], active_id: int) -> InlineKeyboardMarkup:
    buttons = []
    for p in portfolios:
        label = f"{'✅ ' if p['id'] == active_id else ''}{p['name']} — ${p['capital_usdt']:,.0f}"
    ...
    buttons.append([InlineKeyboardButton("🔙 القائمة الرئيسية", callback_data="menu:main")])
    return InlineKeyboardMarkup(buttons)
...
def portfolio_actions_kb(portfolio_id: int, is_active: bool) -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton("🔙 قائمة المحافظ", callback_data="portfolios")])
    return InlineKeyboardMarkup(buttons)
